...
export default function Laundry() {
  return (
    <div className="space-y-8 animate-in fade-in duration-500">
      <header>
        <h2 className="text-3xl md:text-4xl font-bold text-slate-800">Laundry Care</h2>
        <p className="text-lg text-slate-600">Optimized routines to save your favorite pieces.</p>
      </header>
      ...
      >
        ...
        <div>
          <h3 className="text-xl font-bold text-slate-800 mb-2">Ready for a load?</h3>
          ...
          <div className="space-y-4">
            {laundryItems.filter(item => item.status === 'needs-wash').map(item => (
              <div key={item.id} className="bg-white p-4 rounded-xl shadow-sm border border-slate-200">
                ...
                <p className="text-sm text-slate-500 mb-2">Worn {item.lastWorn}</p>
        ...
        <div className="bg-slate-100 rounded-2xl p-6">
          ...
          <div className="space-y-4">
            {laundryItems.filter(item => item.status === 'clean').map(item => (
              <div key={item.id} className="bg-white p-4 rounded-xl shadow-sm border border-slate-200 opacity-60">
                <h4 className="font-bold text-slate-800">{item.name}</h4>
                <p className="text-sm text-slate-500">Ready to wear</p>
              </div>
            ))}
          </div>
        </div>
